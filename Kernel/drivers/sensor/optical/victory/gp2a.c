...
irqreturn_t gp2a_irq_handler(int irq, void *dev_id)
{
    ...
    if(system_rev < 0x0A) //jihyon.82.kim for victory rev 10
      value = gpio_get_value(S5PV210_GPJ0(1));
    else
...
void gp2a_chip_init(void)
{
  ...
  if(system_rev < 0x0A) //jihyon.82.kim for victory rev 10
  {
	...
	set_irq_type(S3C_GPIOINT(J0, 1), IRQ_TYPE_EDGE_BOTH);
   }
  else
  {
    s3c_gpio_cfgpin(S5PV210_GPH0(1), S3C_GPIO_SFN(GPIO_PS_VOUT_AF));
...
static int gp2a_opt_suspend( struct platform_device* pdev, pm_message_t state )
{
	...
	if(!proximity_enable)
	{
		gpio_direction_output(GPIO_ALS_EN, 0);
